...
def iterate_alias(at: Atri, alias_prefix: str, limits: Tuple[int, int], arr: Test, reg: TestRegistry, cb: Callable[[Atri, int, str, List[Test], TestRegistry], None]):
...
status_src = {
    "right": "/app-assets/exclaim.svg",
    "wrong": "/app-assets/wrong.svg",
    "exclaim": "/app-assets/exclaim.svg"
    }

def hide_testitems(at: Atri, index: int, alias: str, arr: List[Test], reg: TestRegistry):
    testitem: at.testitem_1.__class__ = getattr(at, alias)
    testitem.styles.display = "none"

def show_testitems(at: Atri, index: int, alias: str, arr: List[Test], reg: TestRegistry):
    testitem: at.testitem_1.__class__ = getattr(at, alias)
...
def set_testitems(at: Atri, index: int, alias: str, arr: List[Test], reg: TestRegistry):
    ...
    if sorted_test_index < len(arr):
        ...
        # testname/filename
        testname: at.testname_1.__class__ = getattr(at, "testname_" + str(index))
        testname.custom.text = testname
        # #comments
...
def handle_page_request(at: Atri, req: Request, res: Response):
    ...
    all_keys = list(at.__dict__.keys())
    for k in all_keys:
        del at.__dict__[k]
